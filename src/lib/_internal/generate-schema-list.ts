#!/usr/bin/env bun
/** biome-ignore-all lint/suspicious/noConsole: <Codegen script needs console output> */
...
function findDtoFiles(dir: string, files: string[] = []): string[] {
	...
	try {
		...
		for (const item of items) {
			...
			if (item.isDirectory()) {
				// Skip dot-directories and excluded build/output directories
...
/**
 * Extracts DTO names and file info from a DTO file
 */
function extractSchemaInfo(filePath: string): Array<{
	...
	try {
		...
		// Match: const dtoName = ... (must end with "Dto")
		const matches = content.matchAll(/const\s+(\w+Dto)\s*=/g);
...
export function generateSchemaList() {
	...
	// Check for duplicate schema names before deduping
	const schemasByName = new Map<
		string,
		Array<{ name: string; filePath: string; relativePath: string }>
	>();
	for (const schema of allSchemaInfos) {
	...
	for (const [name, schemas] of schemasByName.entries()) {
		if (schemas.length > 1) {
			console.warn(
				`⚠️  Duplicate schema name detected: "${name}"\n` +
					`   Found in ${schemas.length} files:\n` +
					schemas.map((s) => `     - ${s.relativePath}`).join("\n") +
					"\n   Only the first occurrence will be used. Consider renaming to avoid conflicts."
			);
		}
	}

	// Remove duplicates by name (keep first occurrence) and sort
	const uniqueSchemas = [
	...
	// Generate import statements (using regular imports, not type imports, for proper watching)
